...
impl Invaders {
    pub fn new() -> Self {
        ...
        for x in 0..NUM_COLUMNS {
            for y in 0..NUM_ROWS {
                if (x > 1) && (x < NUM_COLUMNS - 2) && (y > 0) && (y < NUM_ROWS / 2) && (x % 2 == 0) && (y % 2 == 0) {
    ...
    pub fn update(&mut self, delta: Duration) -> bool {
        ...
        if self.move_timer.just_finished() {
            ...
            if downwards {
                let new_duration = self.move_timer.duration().as_millis().saturating_sub(250).max(250);
...
impl Drawable for Invaders {
    fn draw(&self, frame: &mut Frame) {
        for invader in self.army.iter() {
            frame[invader.x][invader.y] = if self.move_timer.elapsed().as_secs_f32() / self.move_timer.duration().as_secs_f32() < 0.5 {
                "x"
            } else {
                "+"
            }
